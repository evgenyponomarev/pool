...
foreach ($wallets as $coin => $wallet) {
    ...
    if ($wallet->can_connect() !== true) {
        $log->logFatal(" unable to connect to RPC server $coin, continue to next coin");
    ...
    if ($setting->getValue('disable_manual_payouts') != 1) {
        ...
        if (count($aPayouts) > 0) {
            ...
            foreach ($aPayouts as $aData) {
                $aBalance = $transaction->getBalanceCoin($aData['account_id'], $coin);
                $dBalance = $aBalance['confirmed'];
                $aData['coin_address'] = $user->getCoinAddress($aData['account_id'], $coin);
                ...
                if ($dBalance > $config['txfee']) {
                    ...
                    if (!$oPayout->setProcessed($aData['id'])) {
                        ...
                        $monitoring->sendMail($cron_name, 'E0010', true);
                        continue(2);
                    }

                    $log->logInfo("\t" . $aData['account_id'] . "\t\t" . $aData['username'] . "\t" . $dBalance . "\t\t" . $aData['coin_address']);
                    try {
                        $aStatus = $wallet->validateaddress($aData['coin_address']);
                        if (!$aStatus['isvalid']) {
                    ...
                    try {
                        $txid = $wallet->sendtoaddress($aData['coin_address'], $dBalance - $config['txfee']);
                    } catch (Exception $e) {
                    ...
                    if ($transaction->addTransaction($coin, $aData['account_id'], $dBalance - $config['txfee'], 'Debit_MP', NULL, $aData['coin_address'], $txid) && $transaction->addTransaction($coin, $aData['account_id'], $config['txfee'], 'TXFee', NULL, $aData['coin_address'])) {
    ...
    if ($setting->getValue('disable_auto_payouts') != 1) {
        // Fetch all users balances of this coin
        $users = $transaction->getAPQueueCoin($coin);
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $aUserData) {
                ...
                if ($dBalance > $aUserData['ap_threshold'] && $dBalance > $config['txfee']) {
                    ...
                    try {
                        $aStatus = $wallet->validateaddress($aUserData['coin_address']);
                        if (!$aStatus['isvalid']) {
                            $log->logError('Failed to verify this users coin address, skipping payout');
                            continue;
                        }
                    ...
                    try {
                        $txid = $wallet->sendtoaddress($aUserData['coin_address'], $dBalance - $config['txfee']);
                    } catch (Exception $e) {
                        $log->logError('Failed to send requested balance to coin address, please check payout process. Does the wallet cover the amount?');
                        continue;
                    }
                    ...
                    if ($transaction->addTransaction($coin, $aUserData['id'], $dBalance - $config['txfee'], 'Debit_AP', NULL, $aUserData['coin_address'], $txid) && $transaction->addTransaction($coin, $aUserData['id'], $config['txfee'], 'TXFee', NULL, $aUserData['coin_address'])) {
                        // Mark all older transactions as archived
                        ...
                        if (!$notification->sendNotification($aUserData['id'], 'auto_payout', $aMailData))
                            $log->logError('Failed to send notification email to users address: ' . $aMailData['email']);
                        // Recheck the users balance to make sure it is now 0
                        $aBalance = $transaction->getBalanceCoin($aUserData['id'], $coin);
                        if ($aBalance['confirmed'] > 0) {
                            $log->logFatal('User has a remaining balance of ' . $aBalance['confirmed'] . ' after a successful payout!');
                            $monitoring->sendMail($cron_name, 'E0065', true);
                            continue(2);
