...
foreach($wallets as $coin => $wallet) {
    ...
    if ($wallet->can_connect() !== true) {
      $log->logFatal(" unable to connect to RPC server $coin, exiting");
      ...
      if (count($aPayouts) > 0) {
        ...
        foreach ($aPayouts as $aData) {
          ...
          if ($dBalance > $config['txfee']) {
            ...
            if ($transaction->addTransaction($coin, $aData['account_id'], $dBalance - $config['txfee'], 'Debit_MP', NULL, $aData['coin_address'], $txid) && $transaction->addTransaction($aData['account_id'], $config['txfee'], 'TXFee', NULL, $aData['coin_address'])) {
