...
foreach($wallets as $coin => $wallet) {
    ...
    $aLastBlock = @$block->getLastByCoin($coin);
    ...
    if (empty($aTransactions['transactions'])) {
        ...
        foreach ($aTransactions['transactions'] as $iIndex => $aData) {
            if ( $aData['category'] == 'generate' || $aData['category'] == 'immature' ) {
                $aBlockRPCInfo = $wallet->getblock($aData['blockhash']);
...
// Now with our blocks added we can scan for their upstream shares
    $aAllBlocks = $block->getAllUnsetShareIdByCoin($coin, 'ASC');
    if (empty($aAllBlocks)) {
        ...
        foreach ($aAllBlocks as $iIndex => $aBlock) {
            if (empty($aBlock['share_id'])) {
                ...
                if ($config['block_bonus'] > 0 && !empty($iAccountId) && !$transaction->addTransaction($coin, $iAccountId, $config['block_bonus'], 'Bonus', $aBlock['id'])) {
